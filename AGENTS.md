...
- **VPS:** Hetzner Cloud ARM64 (Ubuntu 22.04) — runs the separate VPS Docker stack, repo at
  `~/SourceRoot/vps` (own README/AGENTS.md — this repo only documents the Tailscale/Caddy
...
| `/docs`                 | main    | Documentation maintenance — one owning doc per fact (README/AGENTS.md/docs/), never restate  |
...
`op`). Full model: `dotfiles/AGENTS.md` → "Tailnet ACL — as code".
...
run `/docs` to keep README/AGENTS.md/docs/ each owning their own facts → review with
