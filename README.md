...
### [redacted] ([redacted]s via VPN)

```bash
# List all [redacted]s
ssh homelab "docker exec [redacted] [redacted]-remote -l"

# Add magnet link
ssh homelab "docker exec [redacted] [redacted]-remote -a 'magnet:?xt=...'"

# Remove [redacted] (keeps files)
ssh homelab "docker exec [redacted] [redacted]-remote -t <id> -r"

# Remove [redacted] AND files
ssh homelab "docker exec [redacted] [redacted]-remote -t <id> -rad"

# Verify VPN IP
ssh homelab "docker exec [redacted] cat /tmp/[redacted]/ip"
ssh homelab "docker exec [redacted] curl -s https://ipinfo.io/ip"

# Interactive [redacted] manager
ssh homelab "cd ~/homelab && ./scripts/[redacted]_manager.py"
```
...
Create the `[redacted]` folders:

```bash
mkdir -p /mnt/hdd/[redacted]/config
mkdir -p /mnt/hdd/[redacted]/cache
```

Change the ownership and permissions of the folders:

```bash
sudo chown -R 1000:1000 /mnt/hdd/[redacted]/config
sudo chown -R 1000:1000 /mnt/hdd/[redacted]/cache
sudo chmod -R 755 /mnt/hdd/[redacted]/config
sudo chmod -R 755 /mnt/hdd/[redacted]/cache
```
...
Start [redacted] using Docker Compose:

```bash
docker-compose up -d
```

Access the [redacted] web interface:

- Open a web browser and navigate to `http://<your-server-ip>:8096`.
- Follow the on-screen instructions to set up [redacted].
    - Username: jkrumm
...
- Under Administration go to Playback:
    ...
    - General:
        - Rename the server to [redacted]

### Enable [redacted] Hardware Acceleration
...
## Setup [redacted] VPN and [redacted]

[redacted] runs through [[redacted]](https://github.com/qdm12/[redacted])'s network stack - an actively maintained, open-source VPN client supporting 30+ providers. No ports exposed - CLI access only via SSH.

### [redacted] Private Key Setup

1. Get the `[redacted]_private_key` from 1Password
2. Add to Doppler as `[redacted]_PRIVATE_KEY`

### Directory Setup

```bash
sudo mkdir -p /mnt/hdd/[redacted]/{config,watch}
sudo mkdir -p /mnt/hdd/[redacted]/downloads/{complete,incomplete}
sudo chown -R 1000:1000 /mnt/hdd/[redacted]
sudo chmod -R 755 /mnt/hdd/[redacted]
```

Start containers:
```bash
doppler run -- docker compose up -d [redacted] [redacted]
```
...
```bash
# Add magnet link
docker exec [redacted] [redacted]-remote -a "magnet:?xt=urn:btih:..."

# List all [redacted]s
docker exec [redacted] [redacted]-remote -l

# Detailed info for [redacted]
docker exec [redacted] [redacted]-remote -t <id> -i

# Remove [redacted] (keeps files)
docker exec [redacted] [redacted]-remote -t <id> -r

# Remove [redacted] and delete files
docker exec [redacted] [redacted]-remote -t <id> -rad
```

### Verify VPN

```bash
# Check [redacted] health and connection
docker exec [redacted] cat /tmp/[redacted]/ip

# Verify external IP (should show VPN IP, not your real IP)
docker exec [redacted] curl -s https://ipinfo.io/ip
```

### [redacted] Configuration

The LinuxServer.io container doesn't support most settings via environment variables. To persist settings, edit `settings.json` while the container is **stopped**:

```bash
# Stop container (required - changes while running are ignored)
docker stop [redacted]

# Edit settings
vim /mnt/hdd/[redacted]/config/settings.json

# Restart container
doppler run -- docker compose up -d [redacted]
```

**Recommended settings** (add/modify in `settings.json`):
...
|---------|-------|-------------|
| `utp-enabled` | `true` | Better NAT traversal when port forwarding unavailable |
| `encryption` | `2` | Required encryption (0=off, 1=preferred, 2=required) |
| `peer-limit-global` | `200` | Max peers across all [redacted]s |
| `peer-limit-per-[redacted]` | `50` | Max peers per [redacted] |

**Why NOT to use SOCKS5 proxy:**
[redacted] offers SOCKS5 as an alternative, but it's a security downgrade:
- No traffic encryption (ISP sees everything)
- No kill switch protection
- No DNS leak protection
- Only hides IP from [redacted] peers

The current [redacted] VPN setup encrypts all traffic with a kill switch - far more secure than SOCKS5.

### Security Features

| Feature | Setting | Description |
|---------|---------|-------------|
| **Kill switch** | Built-in | iptables firewall blocks all traffic if VPN drops |
| **DNS over TLS** | Cloudflare | Encrypted DNS prevents ISP snooping |
| **Block malicious** | On | Blocks known malware domains |
| **Block surveillance** | On | Blocks tracking domains |
| **Block ads** | On | Blocks ad-serving domains |
| **Protocol** | [redacted] | Fast, modern, audited VPN protocol |

### Switching VPN Provider

[redacted] supports 30+ providers. To switch from [redacted]:

```yaml
environment:
  - VPN_SERVICE_PROVIDER=mullvad  # or protonvpn, surfshark, etc.
  - VPN_TYPE=[redacted]
  - [redacted]_PRIVATE_KEY=${[redacted]_PRIVATE_KEY}
  # Provider-specific options vary - see [redacted] wiki
```

See [[redacted] Wiki](https://github.com/qdm12/[redacted]-wiki) for provider-specific configuration.

## [redacted] Manager CLI

A Python CLI tool for managing [redacted] [redacted]s: add new downloads, move completed ones to the [redacted] library with proper naming, or delete unwanted downloads.

### Installation

The script is located at `scripts/[redacted]_manager.py`. Make it executable:

```bash
chmod +x scripts/[redacted]_manager.py
```

**Requirements:**
- Python 3.12+ (stdlib only - no pip dependencies)
- User must be in the `docker` group
- [redacted] container must be running

### Usage

```bash
# Interactive mode (shows status, then menu)
./scripts/[redacted]_manager.py

# Status only (non-interactive, good for quick checks)
./scripts/[redacted]_manager.py -s
./scripts/[redacted]_manager.py --status
```
...
=== [redacted] [redacted] Manager ===
  ...
  ID  Status        Done        Size  Name
----------------------------------------------------------------------
   1  Downloading    45%     2.36 GB  Outer.Banks.S04.COMPLETE.1080p...
   2  Idle          100%     2.21 GB  Wake.Up.Dead.Man.2025.1080p...
----------------------------------------------------------------------
Total: 2 | Completed: 1 | Downloading: 1

Menu:
  [1] Add new [redacted]
  [2] Move completed [redacted]
  [3] Delete [redacted] (dangerous)
  [r] Refresh status
  [q] Quit
```
...
| Feature | Description |
|---------|-------------|
| **Add [redacted]** | Paste magnet link to start a new download |
| **Move to Library** | Move completed downloads to Movies/Shows/Kids folders |
| **Smart Naming** | Auto-suggests clean folder names (e.g., "Movie Name (2024)") |
| **Delete** | Remove [redacted]s with confirmation (requires typing "DELETE") |
| **Status View** | Shows all [redacted]s with progress, colored by status |

**Name Auto-Suggestion Examples:**
- `Outer.Banks.S04.COMPLETE.1080p.NF.WEB-DL...` → `Outer Banks S04`
- `Wake.Up.Dead.Man.2025.1080p.WEBRip...` → `Wake Up Dead Man (2025)`
- `Interstellar.2014.1080p.BluRay.x264...` → `Interstellar (2014)`

**Move Destinations:**
- `/mnt/hdd/Filme/Movies/` - For movies
- `/mnt/hdd/Filme/Shows/` - For TV series
- `/mnt/hdd/Filme/Kids/` - For kids content

### Shell Aliases

Add these to your `~/.bashrc` for convenient access:

```bash
# [redacted] Manager aliases
alias [redacted]='cd /home/jkrumm/homelab && ./scripts/[redacted]_manager.py'
alias [redacted]-status='cd /home/jkrumm/homelab && ./scripts/[redacted]_manager.py -s'
```
...
**Usage after setup:**
```bash
[redacted]         # Open interactive manager
[redacted]-status  # Quick status check
```
